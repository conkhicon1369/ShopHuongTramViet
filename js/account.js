...
// ---- STORAGE KEYS ----
const ACC_KEY   = 'htv_accounts';
const EMAIL_KEY = 'htv_emails';
...
function loadAccounts() {
    ...
    if (saved) {
        ...
        // Merge newly registered users not yet in saved list
        const registered = JSON.parse(localStorage.getItem('registeredUsers') || '[]');
        let changed = false;
        registered.forEach(u => {
            if (!parsed.find(a => a.email === u.email)) {
                parsed.push({ id: u.id || Date.now(), name: u.name, email: u.email, phone: u.phone || '', role: u.role || 'user', status: 'active', spending: 0, joined: u.joined || new Date().toISOString().split('T')[0] });
                changed = true;
            }
        });
        if (changed) localStorage.setItem(ACC_KEY, JSON.stringify(parsed));
        return parsed;
    }
    // First run: build from defaults + registered users
    const base = [...MOCK_ACCOUNTS];
    const registered = JSON.parse(localStorage.getItem('registeredUsers') || '[]');
    registered.forEach(u => {
        if (!base.find(a => a.email === u.email)) {
            base.push({ id: u.id || Date.now(), name: u.name, email: u.email, phone: u.phone || '', role: u.role || 'user', status: 'active', spending: 0, joined: u.joined || new Date().toISOString().split('T')[0] });
        }
    });
    return base;
}
...
function saveAccounts() {
    localStorage.setItem(ACC_KEY, JSON.stringify(allAccounts));
    // Sync status/role back to registeredUsers so login reflects changes
    const registered = JSON.parse(localStorage.getItem('registeredUsers') || '[]');
    let changed = false;
    allAccounts.forEach(acc => {
        const idx = registered.findIndex(u => u.email === acc.email);
        if (idx >= 0) {
            registered[idx] = { ...registered[idx], name: acc.name, phone: acc.phone, role: acc.role, status: acc.status };
            changed = true;
        }
    });
    if (changed) localStorage.setItem('registeredUsers', JSON.stringify(registered));
}

function loadEmails() {
    const saved = localStorage.getItem(EMAIL_KEY);
    return saved ? JSON.parse(saved) : JSON.parse(JSON.stringify(DEFAULT_EMAILS));
}
function saveEmails() { localStorage.setItem(EMAIL_KEY, JSON.stringify(allEmails)); }
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    allEmails   = loadEmails();
    nextAccId   = Math.max(...allAccounts.map(a => a.id || 0), 99) + 1;
...
function saveProfile() {
    ...
    // Sync to allAccounts
    const acc = allAccounts.find(a => a.email === currentUser.email);
    if (acc) { acc.name = currentUser.name; acc.phone = currentUser.phone; saveAccounts(); }

    document.getElementById('sidebarName').textContent        = name;
    document.getElementById('profileDisplayName').textContent  = name;
    const ud = document.getElementById('userNameDisplay');
    if (ud) ud.textContent = name;
...
function changePassword() {
    ...
    if (idx >= 0) { stored[idx].password = nw; }
    else { stored.push({ email: currentUser.email, password: nw }); }
...
function getAllUsersForPw() {
    ...
    stored.forEach(su => {
        const idx = merged.findIndex(u => u.email === su.email);
...
function initAdmin() {
    ...
    document.getElementById('emailSearch').addEventListener('input', () =>
        renderEmails(document.getElementById('emailSearch').value));
...
function renderDashboard() {
    ...
    const rev = allAccounts.reduce((s,a)=>s+(a.spending||0),0);
    ...
    document.getElementById('dashTopSpenders').innerHTML = top.map((acc,i)=>`
        <div class="spender-row">
            ...
            <div class="flex-grow-1">
                ...
                <div class="spend-bar"><div class="spend-fill" style="width:${((acc.spending||0)/maxSpend*100).toFixed(0)}%;"></div></div>
            </div>
    ...
    document.getElementById('dashEmailPreview').innerHTML = unread.length
        ? unread.map(em=>`
            <div class="dash-email-preview">
                <div class="dash-email-avatar">${em.name.charAt(0)}</div>
                <div>
                    <div class="fw-semibold" style="font-size:.88rem;">${em.name}</div>
                    <div style="font-size:.82rem;color:var(--primary-color);">${em.subject}</div>
                    <small class="text-muted">${em.date}</small>
                </div>
            </div>`).join('')
...
function renderAccountsTable(data) {
    ...
    if (!data.length) { tbody.innerHTML='<tr><td colspan="7" class="text-center text-muted py-4">Không có tài khoản nào</td></tr>'; return; }
    tbody.innerHTML = data.map((acc,i)=>`
        <tr>
            ...
            <td>
                <div class="fw-semibold" style="color:var(--primary-color);font-size:.88rem;">${formatCurrency(acc.spending||0)}</div>
                <div class="spend-bar"><div class="spend-fill" style="width:${Math.min((acc.spending||0)/100000,100).toFixed(0)}%;"></div></div>
            </td>
...
function filterAccounts() {
    ...
    renderAccountsTable(allAccounts.filter(a=>
        ...
        (!status||a.status===status)&&(!role||a.role===role)));
}
...
function saveEditAccount() {
    ...
    saveAccounts();
    renderAccountsTable(allAccounts); renderDashboard();
        ...
        <div class="email-row ${em.unread?'unread':''}" onclick="openEmail(${em.id})">
            <div class="d-flex justify-content-between align-items-start">
                <div style="min-width:0;flex:1;">
                    <div class="fw-semibold" style="font-size:.88rem;">${em.unread?'<span class="unread-dot"></span>':''}${em.name}</div>
...
function openEmail(id) {
    const em=allEmails.find(e=>e.id===id); if(!em) return;
    em.unread=false;
    saveEmails();
    renderEmails(); updateEmailBadge(); renderDashboard();
    document.getElementById('emailDetailPane').innerHTML=`
        <div style="padding:24px;">
            <div class="d-flex justify-content-between align-items-start mb-4 flex-wrap gap-2">
                ...
                <button class="btn btn-primary btn-sm" onclick="openReplyModal(${em.id})"><i class="bi bi-reply me-1"></i>Trả lời</button>
            </div>
            ...
            <button class="btn btn-primary btn-sm" onclick="sendQuickReply('${em.from}')"><i class="bi bi-send me-1"></i>Gửi nhanh</button>
        </div>`;
}
...
function updateEmailBadge() {
    ...
    const badge=document.getElementById('emailUnreadBadge'); if(!badge) return;
