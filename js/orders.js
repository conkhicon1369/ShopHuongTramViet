...
// ---- STORAGE KEY ----
const ORDERS_KEY = 'htv_orders';

// ---- DEFAULT DATA ----
const ALL_ORDERS = [
    { id:'ORD-2024-001', customer:'Nguyễn Văn A',   custEmail:'user@email.com',  phone:'0901 234 567', address:'123 Đường ABC, Q1, TP.HCM',           date:'15/01/2024 09:30', payment:'cod',     status:'completed',  subtotal:820000,  shipping:30000, discount:0,      total:850000,  products:[{name:'Nhang Trầm Huế Cao Cấp',qty:2,price:380000,icon:'🌿'},{name:'Nụ Trầm Tự Nhiên',qty:1,price:60000,icon:'🌸'}], trackStep:4 },
    { id:'ORD-2024-002', customer:'Trần Thị Bích',  custEmail:'bich@email.com',  phone:'0902 345 678', address:'456 Đường XYZ, Q3, TP.HCM',           date:'14/01/2024 14:15', payment:'banking', status:'delivering', subtotal:590000,  shipping:30000, discount:50000,  total:570000,  products:[{name:'Nhang Vòng Trầm Hương',qty:3,price:150000,icon:'🌀'},{name:'Trầm Hương Loại A',qty:1,price:140000,icon:'🪵'}], trackStep:3 },
    { id:'ORD-2024-003', customer:'Nguyễn Văn A',   custEmail:'user@email.com',  phone:'0901 234 567', address:'123 Đường ABC, Q1, TP.HCM',           date:'13/01/2024 11:00', payment:'momo',    status:'approved',   subtotal:270000,  shipping:30000, discount:0,      total:300000,  products:[{name:'Nhang Trầm Hương',qty:5,price:54000,icon:'🕯️'}], trackStep:2 },
    { id:'ORD-2024-004', customer:'Phạm Thị Dung',  custEmail:'dung@email.com',  phone:'0904 567 890', address:'321 Đường PQR, Bình Thạnh, TP.HCM',   date:'12/01/2024 16:45', payment:'cod',     status:'pending',    subtotal:465000,  shipping:30000, discount:0,      total:495000,  products:[{name:'Bộ Nhang Quà Tặng',qty:1,price:350000,icon:'🎁'},{name:'Nụ Trầm Tự Nhiên',qty:2,price:57500,icon:'🌸'}], trackStep:0 },
    { id:'ORD-2024-005', customer:'Hoàng Minh Tú',  custEmail:'tu@email.com',    phone:'0905 678 901', address:'654 Đường STU, Gò Vấp, TP.HCM',       date:'11/01/2024 08:20', payment:'banking', status:'pending',    subtotal:1200000, shipping:0,     discount:100000, total:1100000, products:[{name:'Nhang Trầm Huế Cao Cấp',qty:5,price:190000,icon:'🌿'},{name:'Nhang Vòng Trầm Hương',qty:2,price:150000,icon:'🌀'}], trackStep:0 },
    { id:'ORD-2024-006', customer:'Đặng Văn Hùng',  custEmail:'hung@email.com',  phone:'0907 890 123', address:'999 Đường DEF, Tân Phú, TP.HCM',      date:'10/01/2024 13:00', payment:'cod',     status:'pending',    subtotal:380000,  shipping:30000, discount:0,      total:410000,  products:[{name:'Nhang Trầm Huế Cao Cấp',qty:2,price:190000,icon:'🌿'}], trackStep:0 },
    { id:'ORD-2023-020', customer:'Nguyễn Văn A',   custEmail:'user@email.com',  phone:'0901 234 567', address:'123 Đường ABC, Q1, TP.HCM',           date:'28/12/2023 10:30', payment:'cod',     status:'completed',  subtotal:245000,  shipping:30000, discount:0,      total:275000,  products:[{name:'Nhang Trầm Hương',qty:5,price:49000,icon:'🕯️'}], trackStep:4 },
    { id:'ORD-2023-018', customer:'Võ Thị Thu',     custEmail:'thu@email.com',   phone:'0906 789 012', address:'987 Đường VWX, Tân Bình, TP.HCM',     date:'15/12/2023 09:00', payment:'cod',     status:'cancelled',  subtotal:185000,  shipping:30000, discount:0,      total:215000,  products:[{name:'Nhang Trầm Hương',qty:1,price:185000,icon:'🕯️'}], trackStep:-1 },
];
...
function loadOrders() {
    ...
    // First run: deep copy defaults
    return ALL_ORDERS.map(o => ({ ...o, products: o.products.map(p => ({ ...p })) }));
}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.sidebar-nav a[data-panel]').forEach(link => {
        link.addEventListener('click', e => { e.preventDefault(); showPanel(link.dataset.panel, link); });
    });
});
...
function showPanel(name, clickedLink) {
    ...
    else { document.querySelectorAll(`.sidebar-nav a[data-panel="${name}"]`).forEach(a => a.classList.add('active')); }
}
...
function initUser() {
    ...
    const tabs = [
        {key:'all',label:'Tất cả'},{key:'pending',label:'Chờ xử lý'},{key:'approved',label:'Đã duyệt'},
        {key:'delivering',label:'Đang giao'},{key:'completed',label:'Hoàn thành'},{key:'cancelled',label:'Đã hủy'},
    ];
...
function renderAdminFilterTabs() {
    const defs=[
        {key:'all',label:'Tất cả'},{key:'pending',label:'⏳ Chờ duyệt'},{key:'approved',label:'✅ Đã duyệt'},
        {key:'delivering',label:'🚚 Đang giao'},{key:'completed',label:'🎉 Hoàn thành'},{key:'cancelled',label:'❌ Đã hủy'},
    ];
...
function renderAdminOrders(data) {
    ...
    if(!data.length){tbody.innerHTML='<tr><td colspan="8" class="text-center text-muted py-4">Không có đơn hàng nào</td></tr>';return;}
...
function approveOrder(id)  { const o=orders.find(x=>x.id===id); if(!o) return; o.status='approved';   o.trackStep=1; saveOrders(); afterOrderChange(); showToast('Đã duyệt đơn '+id+'!'); }
function deliverOrder(id)  { const o=orders.find(x=>x.id===id); if(!o) return; o.status='delivering'; o.trackStep=3; saveOrders(); afterOrderChange(); showToast('Đơn hàng đang vận chuyển!'); }
function completeOrder(id) { const o=orders.find(x=>x.id===id); if(!o) return; o.status='completed';  o.trackStep=4; saveOrders(); afterOrderChange(); showToast('Đơn hàng đã giao thành công!'); }
function cancelOrder(id)   {
    if(!confirm('Hủy đơn hàng '+id+'?')) return;
    const o=orders.find(x=>x.id===id); if(!o) return;
    o.status='cancelled'; o.trackStep=-1;
    saveOrders(); afterOrderChange(); showToast('Đã hủy đơn hàng!');
}

function afterOrderChange() {
    renderAdminStats(); renderAdminFilterTabs(); applyAdminFilters();
    renderAdminTracking(); updatePendingBadge();
}

function updatePendingBadge() {
    const count=orders.filter(o=>o.status==='pending').length;
    const badge=document.getElementById('pendingBadge'); if(!badge) return;
...
function openOrderModal(id) {
        ...
        <div class="row g-4">
            ...
            <div class="col-12">
                ...
                <table class="table" style="font-size:.88rem;">
                    ...
                    <tbody>${ord.products.map(p=>`<tr><td>${p.icon} ${p.name}</td><td class="text-center">${p.qty}</td><td class="text-end">${p.price.toLocaleString('vi-VN')}đ</td><td class="text-end fw-bold">${(p.price*p.qty).toLocaleString('vi-VN')}đ</td></tr>`).join('')}</tbody>
                    <tfoot>
    ...
    if(ord.status==='pending')    footer.innerHTML+=`<button class="btn btn-success" onclick="approveOrder('${ord.id}');closeOrderModal()"><i class="bi bi-check-circle me-1"></i>Duyệt đơn</button><button class="btn btn-danger" onclick="cancelOrder('${ord.id}');closeOrderModal()"><i class="bi bi-x-circle me-1"></i>Hủy đơn</button>`;
    else if(ord.status==='approved')   footer.innerHTML+=`<button class="btn btn-primary" onclick="deliverOrder('${ord.id}');closeOrderModal()"><i class="bi bi-truck me-1"></i>Chuyển sang giao hàng</button>`;
    else if(ord.status==='delivering') footer.innerHTML+=`<button class="btn btn-success" onclick="completeOrder('${ord.id}');closeOrderModal()"><i class="bi bi-check2-all me-1"></i>Xác nhận đã giao</button>`;
